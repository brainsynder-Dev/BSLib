...
public class Cuboid implements Cloneable {
    ...
    private final List<Material> aboveMaterials = Arrays.asList(
            Material.AIR,
            Material.BARRIER
    );
    ...
    public Location getCenter() {
        ...
        int y1 = this.y2 + 1;
        int z1 = this.z2 + 1;
        return new Location(this.getWorld(), this.x1
                + (x1 - this.x1) / 2.0, this.y1
                + (y1 - this.y1) / 2.0, this.z1
                + (z1 - this.z1) / 2.0);
    }
    ...
    public BlockLocation getRandomLocation(boolean valid, boolean checkInside){
        return getRandomLocation(valid, checkInside, false);
    }
        ...
        if (contains(x, y, z)) {
            ...
            if (valid) {
                ...
                if ((!contains(blockLocation))
                    ...
                    return getRandomLocation(valid, checkInside, allowedInBlocks);
                }
            }

            if (checkInside && isInside(blockLocation)) return getRandomLocation(valid, checkInside, allowedInBlocks);

            if (!allowedInBlocks) {
                Block block = blockLocation.toLocation().getBlock();
                Material type = block.getType();
                if ((type != Material.AIR)
                        || (!type.name().contains("LEAVES"))
                        || (!type.isTransparent())
                        || (!block.isLiquid())) return getRandomLocation(valid, checkInside, allowedInBlocks);
            }
    ...
    public boolean isInside (BlockLocation loc) {
        ...
        // Check if location is 10 blocks near a "roof"
        for (int height = y; height < (loc.getY()+15); height++) {
            Block block = loc.getWorld().getBlockAt(x,height,z);
            if (!contains(block)) break;
            Material type = block.getType();
            if ((type != Material.AIR)
                    || (!type.name().contains("LEAVES"))
                    || (!type.isTransparent())
        ...
        // Checks if location is 20 blocks near a "floor"
        for (int height = y; height > (loc.getY()-15); height--) {
            Block block = loc.getWorld().getBlockAt(x,height,z);
            if (!contains(block)) break;
            Material type = block.getType();
        ...
        // Checks if the location has blocks above and below... means its in a building...
    ...
    public Collection<Entity> getEntities() {
        ...
        getChunks().forEach(chunk -> {
            for (Entity entity : chunk.getEntities()) {
                if (!contains(new BlockLocation(entity.getLocation()))) continue;
