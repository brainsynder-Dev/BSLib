...
import lib.brainsynder.tellraw.Tellraw;
...
public class SubCommand implements CommandExecutor, TabCompleter {
     ...
     */
    public void send(Argument argument, Callback<Player> value) {
     ...
     */
    public void send(Argument argument, Callback<Player> value, Callback<ReturnType> type) {
        List<String> targets = new ArrayList<>();
        argument.target(targets);
     ...
     * @param value    fetched player(s)
     */
    public void send(String argument, Callback<Player> value) {
     ...
     *                 NO_TEAM <If the team selected was not found>
     */
    public void send(String argument, Callback<Player> value, Callback<ReturnType> type) {
        if (argument.equals("@a")) {
            type.dial(ReturnType.SUCCESS);
            Bukkit.getOnlinePlayers().forEach(value::dial);
            return;
        }
        ...
        if (argument.startsWith("team=")) {
            ...
            if (team == null) {
                type.dial(ReturnType.NO_TEAM);
                return;
            }
            type.dial(ReturnType.SUCCESS);
            for (String name : team.getEntries()) {
                Player target = Bukkit.getPlayer(name);
                if (target != null) value.dial(target);
            }
            return;
        }
        ...
        try {
            ...
            if (target == null) {
                type.dial(ReturnType.NO_PLAYER);
                return;
            }
            type.dial(ReturnType.SUCCESS);
            value.dial(target);
